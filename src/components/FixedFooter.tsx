...
export function FixedFooter({ themeColors }: { themeColors: string[] }) {
  ...
  useEffect(() => {
    ...
    const setFromCoords = async (lat: number, lon: number) => {
      // 城市名用逆地理（尽量拿到一个可读的 city），失败也不影响天气查询
      try {
    ...
    const load = async () => {
      ...
      if (navigator.geolocation) {
        try {
          ...
          await setFromCoords(latitude, longitude)
      ...
      try {
        ...
        if (!alive) return
        if (typeof json.city === 'string') setCity(json.city)
  ...
  const renderRow = (
    Icon: ComponentType<{ className?: string; 'aria-hidden'?: boolean; focusable?: string | boolean }>,
    row: PriceRow,
    color: string,
  ) => {
